...
type LockCharacterResponse struct {
	ID      string `json:"ID"`
...
func LockCharacter(a *valorant.Auth, matchId string, characterId string) (*LockCharacterResponse, error) {
	resp, err := utils.PostRequest("/pregame/v1/matches/"+matchId+"/lock/"+characterId, "glz", a)
	if err != nil {
		return nil, err
	}
	respBody := LockCharacterResponse{}
